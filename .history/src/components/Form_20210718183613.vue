...
export default {
  setup(_, { emit }) {
    ...
    const { value, errorMessage } = useField('firstName');
    const { value, errorMessage } = useField('lasttName');
    const { value, errorMessage } = useField('age');
    ...
    const { handleSubmit } = useForm({ vaidationSchema: schema });
    ...
    return { onSubmit, person, onKeyPress, value, errorMessage };
  },
