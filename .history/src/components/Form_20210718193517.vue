...
export default {
  setup() {
    ...
    // });
    const { value: firstName, errorMessage } = useField('firstName');
    const { value: lastName } = useField('lastName');
    const { value: age } = useField('age');
    const { handleSubmit } = useForm();
    const onSubmit = handleSubmit((values,..args) => {
      console.log(values);
      console.log('1');
    });
    ...
    return { onSubmit, onKeyPress, firstName, lastName, age, errorMessage };
  },
